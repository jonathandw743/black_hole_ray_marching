...
fn rd_derivative(ro: vec3<f32>, h2: f32, ro2: vec3f, h22: f32) -> vec3<f32> {
    return u.DISTORTION_POWER * u.RS * -1.5 * h2 * ro / pow(dot(ro, ro), 2.5) + u.DISTORTION_POWER * u.RS * -1.5 * h22 * ro2 / pow(dot(ro2, ro2), 2.5);
}
...
fn get_delta_photon_rk4(photon: Photon, delta_time: f32, h2: f32, h22: f32) -> Photon {
    let p = vec3f(10.0, 0.0, 0.0);
    let ro_k1 = delta_time * photon.rd;
    let rd_k1 = delta_time * rd_derivative(photon.ro, h2, photon.ro - p, h22);
    
    let ro_k2 = delta_time * (photon.rd + 0.5 * rd_k1);
    let rd_k2 = delta_time * rd_derivative(photon.ro + 0.5 * ro_k1, h2, photon.ro - p, h22);
    
    let ro_k3 = delta_time * (photon.rd + 0.5 * rd_k2);
    let rd_k3 = delta_time * rd_derivative(photon.ro + 0.5 * ro_k2, h2, photon.ro - p, h22);
    
    let ro_k4 = delta_time * (photon.rd + rd_k3);
    let rd_k4 = delta_time * rd_derivative(photon.ro + ro_k3, h2, photon.ro - p, h22);
...
fn get_col(initial_photon: Photon) -> vec3<f32> {
    ...
    let initial_ro_rd_cross2 = cross(photon.ro - vec3f(5.0, 0.0, 0.0), photon.rd);
    let h22 = dot(initial_ro_rd_cross2, initial_ro_rd_cross2);
    ...
    for (var i = 0; i < MAX_ITERATIONS; i++) {
        ...
        let dist_to_surfaces = min(dist_to_surfaces, photon_sphere_dist);
        let adjusted_dist_to_surfaces = dist_to_surfaces
        ...
        let delta_photon = get_delta_photon_rk4(photon, dd, h2, h22);
